...
const InvoiceModal = ({ 
  ...
  const generateInvoice = () => {
    // scale: 2 dramatically improves the PDF resolution compared to the default
    html2canvas(document.querySelector("#invoiceCapture"), { scale: 2 }).then((canvas) => {
      ...
      // Dynamically name the file using the invoice number
  ...
  return (
    <Modal show={showModal} onHide={closeModal} size="lg" centered>
      <div id="invoiceCapture">
        <div className="d-flex flex-row justify-content-between align-items-start bg-light w-100 p-4">
          ...
          <div className="text-end ms-4">
            ...
            <h5 className="fw-bold text-primary"> {currency} {total}</h5>
          </div>
        </div>
          ...
          <Table>
            <tbody>
              ...
              <tr className="text-end">
                ...
                <td className="text-end fw-bold" style={{ width: '100px' }}>{currency} {total}</td>
              </tr>
            </tbody>
          </Table>
      ...
      <div className="pb-4 px-4">
        <Row>
          <Col md={6}>
            {/* Note: In a real app, 'Send Invoice' would likely trigger an API call to an email service. 
                For now, it triggers the same download action as the original code. */}
            <Button variant="primary" className="d-block w-100 shadow-sm" onClick={generateInvoice}>
              <BiPaperPlane style={{ width: '15px', height: '15px', marginTop: '-3px' }} className="me-2" />
              Send Invoice
            </Button>
          </Col>
          <Col md={6}>
            <Button variant="outline-primary" className="d-block w-100 mt-3 mt-md-0 shadow-sm" onClick={generateInvoice}>
